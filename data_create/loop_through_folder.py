...
DEFAULT_IMAGE_PATH = os.path.join(IMAGE_DIR, "mask_images", 'default-mask.png')
BLACK_IMAGE_PATH = os.path.join(IMAGE_DIR, "mask_images", 'black-mask.png')
BLUE_IMAGE_PATH = os.path.join(IMAGE_DIR, "mask_images", 'dust.png')
RED_IMAGE_PATH = os.path.join(IMAGE_DIR, "mask_images", 'red-mask.png')
human_path = os.path.join(IMAGE_DIR, 'human_images')
...
def create_mask(image_path):
    pic_path = image_path
    mask_path = "/home/shamim/Downloads/Compressed/face-mask-detector/data_create/images/mask_images/dust.png"
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path, show, model).mask()
    #FaceMasker(pic_path, mask_path, show, model)._save()
...
class FaceMasker:
    ...
    def mask(self):
        ...
        face_path = glob.glob(self.face_path + '/*.jpg')
    
       
        for face in face_path:
            face_image_np = face_recognition.load_image_file(face)
            ...
            if found_face:
                ...
                # save
                path_splits = os.path.splitext(face)
                new_face_path = path_splits[0] + '-with-mask' + path_splits[1]
                print(new_face_path)
                new_face_path = os.path.join("data_generated", new_face_path)
    ...
    def _save(self):
        ...
        #new_face_path = os.path.join(IMAGE_DIR, "/output", "path_splits[0] + '-with-mask' + path_splits[1]") #path_splits[0] + '-with-mask' + path_splits[1]
        new_face_path = path_splits[0] + '-with-mask' + path_splits[1]
        #print(new_face_path)
        self._face_img.save(new_face_path)
        print(f'Save to {new_face_path}')
